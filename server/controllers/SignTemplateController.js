...
export default class SignTemplateController {
  ...
  async edit(req, res, next) {
    try {
      req.siteId = mongodb.ObjectID(req.query.siteId)
      let data = await _signTemplateRepo.findOneAndUpdate({ _id: req.params.id }, req.body, { new: true })
      if (data) {
        return res.send(data)
      }
  ...
  async delete(req, res, next) {
    try {
      req.siteId = mongodb.ObjectID(req.query.siteId)
      await _signTemplateRepo.findOneAndRemove({ _id: req.params.id, authorId: req.session.uid })
