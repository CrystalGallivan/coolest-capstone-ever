<template>
  <div class="costing contianer-fluid">
    ...
    <div class="row">
      <edit-recipe-form v-if="activeRecipe._id || activeRecipe.id" />
      <new-recipe-form v-else />
    </div>
    <!-- Ingredient Table Template -->
    <div class="row" v-if="activeRecipe._id || activeRecipe.id">
      <add-ingredients />
    </div>
    <div class="row" v-if="activeRecipe._id || activeRecipe.id">
      <sub-recipe />
      ...
      <div class="row menu-description">
        <div class="col-10 offset-1">
          ...
          <textarea
            ...
            class="form-control"
            rows="3"
            v-model="activeRecipe.menuDescription"
            >{{ this.activeRecipe.menuDescription }}</textarea
          >
        </div>
      </div>
      <div class="row directions">
        <div class="col-10 offset-1">
          ...
          <textarea
            ...
            class="form-control"
            rows="3"
            v-model="activeRecipe.directions"
            >{{ this.activeRecipe.directions }}</textarea
          >
        </div>
      </div>
      <div class="row comments">
        <div class="col-10 offset-1">
          ...
          <textarea
            ...
            class="form-control"
            rows="3"
            v-model="activeRecipe.comments"
            >{{ this.activeRecipe.comments }}</textarea
          >
        </div>
      </div>
    ...
    <div class="row recipeCalcs">
      ...
      <div class="col-3 text-white d-flex justify-content-center text-left">
        <ul>
          ...
          <li class="mt-2" v-model="activeRecipe.costPerRecipe">
            $ {{ this.recipeCost }}
          </li>
          <li>
            $
            <input
              type="number"
              placeholder="0.00"
              class="totalP-input ml-1 mt-2"
              v-model="activeRecipe.salesPrice"
              required
            />
          </li>
          <li class="mt-2">
            $ {{ (this.activeRecipe.salesPrice - this.recipeCost).toFixed(2) }}
          </li>
    ...
    <!-- Save Recipe Button -->
    <div
      v-if="activeRecipe._id || activeRecipe.id"
      class="form-row d-flex justify-content-center align-content-center"
    >
      <button @click="saveRecipe" type="button" class="btn saveRecipe-btn">
        Save Recipe
      </button>
    </div>
  </div>
...
export default {
  ...
  computed: {
    ...
    recipeCost() {
      return (this.activeRecipe.costPerRecipe = (
        (this.storeroom +
          this.meat +
          this.dairy +
          this.produce +
    ...
    subRecipes() {
      ...
      if (this.activeRecipe.subRecipe != null) {
        let subRecipeCost = this.activeRecipe.subRecipe.forEach(
          (r) => (subRecipe += r.costPerRecipe)
        );
        return subRecipe;
      } else {
        return 0;
    ...
    itemCost() {
      ...
      return this.recipeIngredients.forEach(
        (r) =>
          (this.$data[r.category.toLowerCase()] += +r.itemCost * r.quantity)
      );
    },
    foodCost() {
      //totalCost / portions
  ...
  methods: {
    saveRecipe() {
      ...
      if (this.activeRecipe.recipeIngredients.length > 0) {
        ...
        this.$store.dispatch("editRecipeInCosting", this.activeRecipe);
      }
    },
  },
  components: {
